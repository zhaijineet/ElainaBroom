...
public class ClientEventHandler {
    ...
    public static void handlerRegisterKeyMappingsEvent(RegisterKeyMappingsEvent event) {
        event.registerCategory(KeyMappings.BROOM_CATEGORY);
    ...
    public static void handlerInputEvent$Key(InputEvent.Key event) {
        if (event.getAction() != InputConstants.PRESS) return;
        KeyMappings.customKeyTrigger(InputConstants.getKey(event.getKeyEvent()));
    }

    /**
     * 每个客户端 tick 开始时，将按键输入状态同步到扫帚实体
     */
    public static void handlerClientTickEvent$Pre(ClientTickEvent.Pre event) {
