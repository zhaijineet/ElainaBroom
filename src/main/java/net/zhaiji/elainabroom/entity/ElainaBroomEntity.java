...
public class ElainaBroomEntity extends Entity {
    ...
    public ElainaBroomEntity(EntityType<?> pEntityType, Level pLevel) {
    ...
    public static ElainaBroomEntity summonBroom(ServerLevel serverLevel, ItemStack stack, BlockPos pos, @Nullable Player player) {
        ElainaBroomEntity broom = ElainaBroomEntity.TYPE.create(serverLevel, stack.getTag(), entity -> {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(DATA_MAX_LEVEL, 30);
        this.entityData.define(DATA_SPEED, 1.0F);
        this.entityData.define(DATA_FRICTION, 0.93F);
        this.entityData.define(DATA_FORWARD_SPEED, 2.0F);
        this.entityData.define(DATA_BACK_SPEED, 1.0F);
        this.entityData.define(DATA_LATERAL_SPEED, 1.0F);
        this.entityData.define(DATA_VERTICAL_SPEED, 1.2F);
    }
    ...
    public InteractionResult interact(Player pPlayer, InteractionHand pHand) {
        if (!pPlayer.isShiftKeyDown() && this.canAddPassenger(pPlayer)) {
            if (pPlayer.experienceLevel < this.getNeedLevel()) {
                if (this.level().isClientSide()) {
                    pPlayer.displayClientMessage(Component.translatable("tips.elainabroom.need_level", this.getNeedLevel()), true);
                }
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (this.level().isClientSide || this.isRemoved()) {
            ...
            return false;
        } else if (pSource.getEntity() instanceof Player player && player.isShiftKeyDown()) {
